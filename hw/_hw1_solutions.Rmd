...
1. Create three vectors (`weekday`, `poker`, `roulette`) to describe the following outcome. Be sure to use these vector names exactly as I have written them here. 
...
sum(poker) > sum(roulette)
```
Lost $84 overall. Did better on poker.
...
```{r}
colSums(star_wars_matrix[c(4,5,6,7,8), ])
```
...
> If that didn't work, did you name your vectors earlier exactly `weekday`, `poker`, and `roulette`?


2. Print out the records for just Tuesday and Thursday using bracket notation 
and indices `[,]`. _Hint: To print out the first two columns you could use [,c(1,2)]_
```{r}
vegas[c(2,4),]
```
...
3. Print out the records for Wednesday only, using bracket notation and a logical statement such
as `vegas$day == "Wed"` to identify the row of interest. 
```{r}
vegas[vegas$day== "Wednesday",]
```


4. Create a new variable called `vegas$total` that adds the daily totals for each
game together. Print the full data set again. Notice that the `total` variable is now 
part of the `vegas` dat set. 
```{r}
vegas$total <- rowSums(vegas[,c(2,3)])
```
...
`vegas$win_days`, and print this new object to the screen. 
```{r}
vegas[vegas$total > 0, ]
```
